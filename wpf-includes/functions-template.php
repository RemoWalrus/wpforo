	...
	if( $forum_profile ) {
        $menu_html .= '<div id="wpf-pp-forum-menu" class="wpf-pp-menu">
            <div class="wpf-pp-menu-item">
                <i class="fas fa-comments" title="' . wpforo_phrase('Room Profile', false) . '"></i> <span>' . wpforo_phrase('Room Profile', false) . '</span>
            </div>
        </div>';
...
class wpForo_Widget_search extends WP_Widget {
	...
	public function form( $instance ) {
		$title = isset( $instance['title'] ) ? $instance['title'] : 'Room Search';
		?>
		<p>
...
class wpForo_Widget_recent_topics extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			array( 'description' => 'Your room\'s recent topics.' ) // Args
		);
		$this->init_local_vars();
	}
	private function init_local_vars(){
...
//		wp_enqueue_script('wpforo-widgets-js');
	...
	public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('forumids_filter') ?>"><?php _e('Filter by rooms', 'wpforo'); ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('current_forumid_filter') ?>"><?php _e('Autofilter by current room', 'wpforo'); ?>:</label>
...
class wpForo_Widget_recent_replies extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			array( 'description' => 'Your room\'s recent posts.' ) // Args
		);
		$this->init_local_vars();
	}
	private function init_local_vars(){
...
//		wp_enqueue_script('wpforo-widgets-js');
	...
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : 'Rooms';
		$display_avatar = isset( $instance['display_avatar'] ) ? (bool) $instance['display_avatar'] : false;
		?>
		<p>
			<label><?php _e('Title', 'wpforo'); ?>:</label> 
...
function wpforo_activity_like( $item = array() ){
	...
	if( isset($item['title']) ) $args['title'] = preg_replace('|^.+?\:\s*|is', '', $item['title']);
	if( $args['title'] ) $args['title'] = ' "' . esc_html($args['title']) . '"';
	$args['action'] = sprintf( wpforo_phrase('%s liked room post %s', false), '', '');
	wpforo_activity( $args );
}
        ...
        <div class="wpf-admincp">
            ...
            <div class="wpf-acp-body" style="display: <?php echo $display ?>;">
                <div class="wpf-acp-content">
                    <a href="<?php echo admin_url('admin.php?page=wpforo-forums'); ?>" class="wpf-button-secondary"><i class="fas fa-plus"></i>&nbsp; <?php wpforo_phrase('Add New Category or Room') ?></a>
                    <p class="wpf-acp-forum-info">
				        ...
				        ?>
				        <?php echo sprintf( wpforo_phrase('Please note, that rooms can be displayed with different layouts %1$s, just edit the top category (blue panel) and set %2$s. Child rooms inherit the top category (blue panel) layout.', false), $layouts, $layout ); ?>
                    </p>
                </div>
                <div class="wpf-acp-footer">
			        ...
			        <?php if(!empty($menu) && isset($menu->term_id)): ?>
                        <a href="<?php echo admin_url('nav-menus.php?action=edit&menu=' . intval($menu->term_id) ); ?>" class="wpf-button-secondary"><i class="fas fa-bars"></i> <?php wpforo_phrase('Room Menu') ?></a>
			        <?php endif; ?>
                    <a href="<?php echo admin_url('widgets.php'); ?>" class="wpf-button-secondary"><i class="far fa-window-restore"></i> <?php wpforo_phrase('Room Widgets') ?></a>
                    <a href="<?php echo wp_nonce_url(wpforo_home_url('?wpfaction=wpforo_reset_all_caches'), 'wpforo_reset_cache') ?>" class="wpf-button-secondary" style="color: #e22d00 !important;"><i class="fas fa-times"></i> <?php wpforo_phrase('Delete Room Cache') ?></a>
                </div>
            </div>
        </div>
        <?php
